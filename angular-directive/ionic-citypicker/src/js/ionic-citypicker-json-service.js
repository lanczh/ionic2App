"use strict";
var app = angular.module('ionic-citypicker', ['ionic', 'ionic-citypicker.service']);
app.directive('ionicCityPicker', ['$ionicPopup', '$timeout','CityPickerService','$ionicScrollDelegate','$ionicModal', function ($ionicPopup, $timeout,CityPickerService, $ionicScrollDelegate,$ionicModal) {
  ...
  return {
    restrict: 'AE',
    template:  '<div class={{vm.cssClass}}><div class={{vm.iconClass}}>{{vm.areaData}}</div><span class={{vm.spanClass}}></span></div></div>',
    scope: {
      options : '=options'
    },
...
//        vm.cssClass = 'ionic-citypicker item'  + (angular.isDefined(so.iconClass) ? ' item-icon-left ' : ' ') + (angular.isDefined(so.cssClass) ? so.cssClass : '')
        vm.cssClass = 'ionic-citypicker'  + (angular.isDefined(so.iconClass) ? ' ' : ' ') + (angular.isDefined(so.cssClass) ? so.cssClass : '')
        vm.iconClass = 'icon ' + (angular.isDefined(so.iconClass) ? so.iconClass :'')
        vm.spanClass = 'all_arrow_down ' + (angular.isDefined(so.spanClass) ? so.spanClass :'')
        ...
        vm.returnOk = function(){
          ...
          so.areaData = vm.areaData.split(vm.tag)
          so.provinceCode = vm.province.value
          so.countryCode = vm.country.value
          so.cityCode = vm.city.value
          //判断是否显示区
          if(vm.AreaQu==""){
        	  
          }else if(vm.AreaQu=="1"){
        	  vm.areaData = vm.province.name
          }else if(vm.AreaQu=="2"){
        	  vm.areaData = vm.city.name
          }else if(vm.AreaQu=="3"){
        	  vm.areaData = vm.country.name
          }
          ...
          scope.$watch('options.areaData', function(newVal,oldVal){
              if (newVal !== oldVal && isArray(newVal) && newVal.length > 1 && newVal.join(vm.tag) !== vm.areaData) {
                if (vm.isCreated) {
				 ...
				 //判断是否显示区
                var centerArea=vm.areaData;
	  	          if(vm.AreaQu==""){
	  	        	  newVal = so.areaData;
	  	          }else if(vm.AreaQu=="1"){
	  	        	  centerArea = so.areaData[0]
	  	        	  newVal=[centerArea]
	  	          }else if(vm.AreaQu=="2"){
	  	        	  centerArea = so.areaData[1]
	  	        	  newVal=[centerArea]
	  	          }else if(vm.AreaQu=="3"){
	  	        	  centerArea = so.areaData[2]
	  	        	  newVal=[centerArea]
	  	          }
...
//
app.directive('ionicCityPicker2', ['$ionicPopup', '$timeout','CityPickerService','$ionicScrollDelegate','$ionicModal', function ($ionicPopup, $timeout,CityPickerService, $ionicScrollDelegate,$ionicModal) {
	  function isBoolean(value) {return typeof value === 'boolean'}
	  function isArray(value) {return toString.apply(value) === '[object Array]'}
	  return {
	    restrict: 'AE',
	    template:  '<div class={{vm.cssClass}}><div class={{vm.iconClass}}>{{vm.title}}</div><span class={{vm.spanClass}}>{{vm.areaData}}</span></div></div>',
	    scope: {
	      options : '=options'
	    },
	    link: function (scope, element, attrs) {
...
//	        vm.cssClass = 'ionic-citypicker item'  + (angular.isDefined(so.iconClass) ? ' item-icon-left ' : ' ') + (angular.isDefined(so.cssClass) ? so.cssClass : '')
	        ...
	        so.areaData = vm.areaData.split(vm.tag)
	        if(vm.AreaQu=='2'){
	        	vm.areaData=so.areaData[1];
	        }
	        vm.returnOk = function(){
	          (vm.city && vm.city.sub && vm.city.sub.length > 0) ? (vm.areaData = vm.province.name + vm.tag +  vm.city.name + vm.tag ) : (vm.areaData = vm.province.name + vm.tag + vm.city.name)
	          so.areaData = vm.areaData.split(vm.tag)
	          so.provinceCode = vm.province.value
          	  so.cityCode = vm.city.value
	          //判断是否显示区
	        ...
	        vm.getValue = function(name) {
	          ...
	          switch(name)
	          {
	            ...
	            case 'city':
	              if (!vm.province.sub) {alert('city数据出错')}
	              var city = true, Handle = vm.cityHandle, HandleChild = null
	            break
	          }
	          var top = $ionicScrollDelegate.$getByHandle(Handle).getScrollPosition().top // 当前滚动位置
	          var step = Math.round(top / vm.step)
	          ...
	          vm.runing = $timeout(function () {
	            province && (vm.province = vm.AreaService[step], vm.city = vm.province.sub[0],(vm.city && vm.city.sub )) //处理省市乡联动数据
	            city &&  (vm.city = vm.province.sub[step]) // 处理市联动数据
	            HandleChild && $ionicScrollDelegate.$getByHandle(HandleChild).scrollTop() // 初始化子scroll top位
	          })
	        }
	        vm.initAreaData = function(AreaData) {
	        ...
	        element.on("click", function () {
	            ...
	            vm.isCreated = true
	            // $ionicModal.fromTemplateUrl('lib/ionic-citypicker/src/templates/ionic-citypicker.html', {
	            $ionicModal.fromTemplateUrl('lib/ionic-citypicker/src/templates/ionic-citypicker2.html', {
	              scope: scope,
	              animation: 'slide-in-up',
	              backdropClickToClose: vm.backdropClickToClose,
	              hardwareBackButtonClose: vm.hardwareBackButtonClose,
